	...
	public static EASYBUY_USER selectById(String id) {
		...
		try {
			ps = conn.prepareStatement(
					"select m.*,to_date(m.eu_birthday,'%Y-%m-%d')birthday from EASYBUY_USER m where EU_USER_ID=?");
			ps.setString(1, id);
			rs = ps.executeQuery();
			while (rs.next()) {
...
//		u.setEU_USER_NAME("testwu1");
		String sql = "insert into EASYBUY_USER values(?,?,?,?,now(),?,?,?,?)";
		Object[] params = { u.getEU_USER_ID(), u.getEU_USER_NAME(), u.getEU_PASSWORD(), u.getEU_SEX(),
				u.getEU_EMAIL(), u.getEU_MOBILE(), u.getEU_ADDRESS(),
				u.getEU_STATUS() };
		return Basedao.exectuIUD(sql, params);
	}

	public static int update(EASYBUY_USER u) {
		if("1".equals(u.getEU_USER_NAME()))
		{
			String sql = "update EASYBUY_USER set EU_USER_NAME=?," + "EU_PASSWORD=?," + "EU_SEX=?,"
					+ "EU_BIRTHDAY=to_date(?,'%Y-%m-%d')," + "EU_EMAIL=?," + "EU_MOBILE=?,"
